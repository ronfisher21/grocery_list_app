...
# Old → new category name mapping. Applied everywhere data is read from cache/DB
# so stale entries from SQLite, manual_overrides, or grocery_items are auto-corrected.
CATEGORY_MIGRATIONS: dict[str, str] = {
    "מוצרי חלב וביצים": "מוצרים לאחסן במקרר",
}


def migrate_category(category: str) -> str:
...
8. קלט: "יוגורט" -> קטגוריה: מוצרים לאחסן במקרר
...
- מוצרים לאחסן במקרר: מילקי, דני, לבן, קוטג', שמנת, גבינה, יוגורט, אשל, חמאה, ביצים, גבינה צהובה, בולגרית, פטה
...
- ניקיון: אקונומיקה, סבון, ג'ל כלים, מרכך, אבקת כביסה, נייר טואלט, מגבונים, מגבונים צהובים, ריחן לשירותים
...
- אם אתה לא בטוח, שייך לקטגוריה הקרובה ביותר (למשל: סבון כלים לניקיון, קוטג' למוצרים לאחסן במקרר).
- **עבור שמות בעלי יותר מ-1 מילה: בחן כל מילה בהקשר המלא של המוצר.** דוגמאות:
  - "מגבונים צהובים" = מגבונים (מוצר ניקיון, מגבת) + צהובים (צבע) → ניקיון
  - "פלפל אדום" = פלפל (ירק) + אדום (צבע) → ירקות ופירות
  - "חלב טרי" = חלב (מוצר לאחסן במקרר) + טרי (טיכסה) → מוצרים לאחסן במקרר
  - לא תמיד צבע או תיאור משנים את הקטגוריה - התמקד במילה הראשונה/עיקרית.
- שם המוצר עשוי להכיל מספר בתחילתו או בסופו המציין כמות (למשל "3 ביצים", "חלב 2"). התעלם מהמספר לחלוטין וסווג לפי שם המוצר בלבד. דוגמאות: "3 ביצים" -> מוצרים לאחסן במקרר, "2 לחמניות" -> לחם, "5 תפוחים" -> ירקות ופירות.
"""
